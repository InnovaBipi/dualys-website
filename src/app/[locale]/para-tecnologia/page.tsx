...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const content = await getPageContent<LandingContent>('landing_tecnologia', locale as Locale);
  return generatePageMetadata({ title: content.meta.title, description: content.meta.description, locale: locale as Locale, path: '/para-tecnologia' });
}

export default async function ParaTecnologiaPage({ params }: PageProps) {
  const { locale } = await params;
  setRequestLocale(locale);
  const content = await getPageContent<LandingContent>('landing_tecnologia', locale as Locale);
  const homepage = await getPageContent<HomepageContent>('homepage', locale as Locale);
  const audienceCard = homepage.audience.cards.find((c) => c.key === 'tech');

  const pageSchema = getWebPageSchema({ title: content.meta.title, description: content.meta.description, locale: locale as Locale, path: '/para-tecnologia' });
  const breadcrumbSchema = getBreadcrumbSchema([{ name: 'Home', url: '' }, { name: content.title, url: '/para-tecnologia' }], locale as Locale);
        ...
        <Container>
          <div className="mx-auto max-w-3xl text-center">
            <div className="mb-6 inline-flex h-14 w-14 items-center justify-center rounded-full bg-accent-500/20">
              <Cpu className="h-7 w-7 text-accent-400" aria-hidden="true" />
            </div>
