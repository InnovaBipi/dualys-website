...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const t = await getTranslations({ locale, namespace: 'landings.tecnologia' });

  return generatePageMetadata({
    title: t('meta.title'),
    description: t('meta.description'),
    locale: locale as Locale,
    path: '/para-tecnologia',
  });
}
...
export default async function ParaTecnologiaPage({ params }: PageProps) {
  ...
  const breadcrumbSchema = getBreadcrumbSchema([
    { name: 'Home', url: '' },
    { name: t('title'), url: '/para-tecnologia' },
  ], locale as Locale);
        ...
        <Container>
          <div className="mx-auto max-w-3xl text-center">
            <h2 className="text-2xl font-bold tracking-tight text-primary-950 sm:text-3xl">
              {tAudience('tech.title')}
            </h2>
            <p className="mt-4 text-lg leading-relaxed text-neutral-600">
              {tAudience('tech.description')}
            </p>
          </div>
        </Container>
      </section>
