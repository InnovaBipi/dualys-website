...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const content = await getPageContent<LandingContent>('landing_industriales', locale as Locale);
  return generatePageMetadata({ title: content.meta.title, description: content.meta.description, locale: locale as Locale, path: '/para-industriales' });
}

export default async function ParaIndustrialesPage({ params }: PageProps) {
  const { locale } = await params;
  setRequestLocale(locale);
  const content = await getPageContent<LandingContent>('landing_industriales', locale as Locale);
  const homepage = await getPageContent<HomepageContent>('homepage', locale as Locale);
  const audienceCard = homepage.audience.cards.find((c) => c.key === 'industrial');

  const pageSchema = getWebPageSchema({ title: content.meta.title, description: content.meta.description, locale: locale as Locale, path: '/para-industriales' });
  const breadcrumbSchema = getBreadcrumbSchema([{ name: 'Home', url: '' }, { name: content.title, url: '/para-industriales' }], locale as Locale);
        ...
        <Container>
          <div className="mx-auto max-w-3xl text-center">
            <div className="mb-6 inline-flex h-14 w-14 items-center justify-center rounded-full bg-accent-500/20">
              <Factory className="h-7 w-7 text-accent-400" aria-hidden="true" />
            </div>
