...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const t = await getTranslations({ locale, namespace: 'landings.industriales' });

  return generatePageMetadata({
    title: t('meta.title'),
    description: t('meta.description'),
    locale: locale as Locale,
    path: '/para-industriales',
  });
}
...
export default async function ParaIndustrialesPage({ params }: PageProps) {
  ...
  const breadcrumbSchema = getBreadcrumbSchema([
    { name: 'Home', url: '' },
    { name: t('title'), url: '/para-industriales' },
  ], locale as Locale);
        ...
        <Container>
          <div className="mx-auto max-w-3xl text-center">
            <h2 className="text-2xl font-bold tracking-tight text-primary-950 sm:text-3xl">
              {tAudience('industrial.title')}
            </h2>
            <p className="mt-4 text-lg leading-relaxed text-neutral-600">
              {tAudience('industrial.description')}
            </p>
          </div>
        </Container>
      </section>
